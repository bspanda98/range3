...
func TestMasternodeService(t *testing.T) {
	...
	delegatedPOS := []common.Address{
		...
		params.RangeTestnetChainConfig.Range.CPPSigner,
	}
	...
	for _, addr := range delegatedPOS {
		...
		switch addr {
		case energi_params.Range_MigrationContract:
			mgSigner = accAddr

		case params.RangeTestnetChainConfig.Range.CPPSigner:
			cpSigner = accAddr
		}
	}
	...
	err := migrations.PrepareTestGen2Migration(params.RangeTestnetChainConfig.ChainID.Uint64())
...
func serviceConfig(presale core.GenesisAlloc) *eth.Config {
	...
	ethConfig.Genesis = core.DefaultRangeTestnetGenesisBlock()
	ethConfig.Genesis.GasLimit = gasLimit
	ethConfig.Genesis.Config.ChainID = big.NewInt(1)
	ethConfig.Genesis.Config.Range.CPPSigner = cpSigner
	ethConfig.Genesis.Config.Range.MigrationSigner = mgSigner
	ethConfig.Genesis.Alloc = presale
	ethConfig.Genesis.Xfers = core.DeployRangeGovernance(ethConfig.Genesis.Config)
	ethConfig.Genesis.Difficulty = big.NewInt(1)
	ethConfig.Genesis.Coinbase = energi_params.Range_Treasury
...
func cpPropose() (*core.CheckpointInfo, error) {
	...
	cpRegistry, err := energi_abi.NewCheckpointRegistryV2(
		energi_params.Range_CheckpointRegistry, ethServ.APIBackend,
	)
	if err != nil {
		return nil, err
	}
...
func depositCollateral(
	...
	mnRegistry, err := energi_abi.NewIMasternodeTokenTransactor(
		energi_params.Range_MasternodeToken, ethService.APIBackend)
	if err != nil {
		return err
	}
...
func announceMN(
	...
	registry, err := energi_abi.NewIMasternodeRegistryV2Transactor(
		energi_params.Range_MasternodeRegistry, ethService.APIBackend)
	if err != nil {
		return err
	}
...
func isSignedCheckpointTx(tx *types.Transaction) bool {
	if *tx.To() == energi_params.Range_CheckpointRegistry {
