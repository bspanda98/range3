...
func (e *Range) processBlacklists(
	chain ChainReader,
	header *types.Header,
	statedb *state.StateDB,
) (err error) {
	blregistry := energi_params.Range_BlacklistRegistry
	...
	state_obj := statedb.GetOrNewStateObject(energi_params.Range_Blacklist)
	db := statedb.Database()
	...
	//
	wl_state_obj := statedb.GetOrNewStateObject(energi_params.Range_Whitelist)
...
var (
	...
	consensusStandalone = []common.Address{
		...
		energi_params.Range_SystemFaucet,
	}
)

func (e *Range) createWhitelist(
	statedb *state.StateDB,
...
func (e *Range) processDrainable(
	...
	for _, addr := range *address_list {
		...
		statedb.SetState(energi_params.Range_Blacklist, addr.Hash(), common.Hash{})
		evm = e.createEVM(msg, chain, header, statedb)
		gp = core.GasPool(msg.Gas())
		_, gas1, failed, err := core.ApplyMessage(evm, msg, &gp)
		statedb.SetState(energi_params.Range_Blacklist, addr.Hash(), blacklistValue)
