...
var (
	...
	//
	energiAuthorsFileHeader = "# This is the official list of Range Core authors for copyright purposes.\n\n"
	energiAuthorsRE         = regexp.MustCompile(`@(energi.team|futoin)`)

	// The commit after which the Range source starts
	//energiForkPoint = "4bcc0a37ab70cb79b16893556cffdaad6974e7d8"

	// Date of Range fork refactoring start
...
// Copyright {{.YearRange}} The Range Core Authors
// Copyright {{.Year}} The go-ethereum Authors
// This file is part of {{.WholeRange false}}.
//
// {{.WholeRange true}} is free software: you can redistribute it and/or modify
// it under the terms of the GNU {{.License}} as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// {{.WholeRange true}} is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
// GNU {{.License}} for more details.
//
// You should have received a copy of the GNU {{.License}}
// along with {{.WholeRange false}}. If not, see <http://www.gnu.org/licenses/>.

`[1:]))

var onlyRangeLicenseT = template.Must(template.New("").Parse(`
// Copyright {{.YearRange}} The Range Core Authors
// This file is part of {{.WholeRange false}}.
//
// {{.WholeRange true}} is free software: you can redistribute it and/or modify
...
func (i info) WholeRange(startOfSentence bool) string {
	if i.gpl() {
		return "Range Core"
	}
	if startOfSentence {
		return "The Range Core library"
	}
	return "the Range Core library"
}
...
func fileInfo(file string) (*info, error) {
	info := &info{file: file, Year: int64(time.Now().Year()), IsRange: false}
	...
	if info.Year >= energiStartYear {
		info.YearRange = info.Year
		// TODO: revise on cherry-picks
		info.Year = energiStartYear
	} else {
		info.YearRange = energiStartYear
	}
	return info, err
}
...
func writeLicense(info *info) {
	...
	if info.IsRange {
		if info.OnlyRange {
			onlyRangeLicenseT.Execute(buf, info)
		} else {
