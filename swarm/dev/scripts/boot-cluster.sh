...
#
# The cluster contains a bootnode, a range3 node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the range3 and
# swarm nodes
start_bootnode() {
...
# start_range3_node starts a range3 node with --datadir pointing at <base-dir>/range3
# and a single, unlocked account with password "range3"
start_range3_node() {
  local dir="${base_dir}/range3"
  mkdir -p "${dir}"

  local password="range3"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating range3 account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get range3 account address"
  fi
  ...
  start_node "range3" "${GETH_IP}" "$(which range3)" ${args[@]}
}
...
create_account() {
  ...
  range3 --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
