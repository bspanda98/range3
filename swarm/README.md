...
To run Swarm you need an Range account. You can create a new account by running the following command:
...
Once you have specified the password, the output will be the Range address representing that account. For example:
...
### Range Name Service resolution

The Range Name Service is the Range equivalent of DNS in the classic web. In order to use ENS to resolve names to Swarm content hashes (e.g. `bzz://theswarm.eth`), `swarm` has to connect to a `range3` instance, which is synced with the Range mainnet. This is done using the `--ens-api` flag.

    swarm --bzzaccount <your-account-here> \
          --ens-api '$HOME/.ethereum/range3.ipc'
