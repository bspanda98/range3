// Copyright 2020 The Range Core Authors
...
func (w *worker) tryAutocollateral() {
	...
	if _, ok := w.engine.(*energi.Range); !ok {
		// Range consensus engine not running.
	...
	// Get rewards
	mnReward, err := w.getBlockReward(energi_params.Range_MasternodeRegistry, block.Number())
	if err != nil {
		log.Error(err.Error())
		return
	}
...
func (w *worker) tokenRegistry(dst common.Address) (*energi_abi.IMasternodeTokenSession, error) {
	contract, err := energi_abi.NewIMasternodeToken(
		energi_params.Range_MasternodeToken, w.apiBackend.(bind.ContractBackend))
	if err != nil {
		return nil, err
	}
...
func (w *worker) collateralLimits() (minCollateral, maxCollateral *big.Int, err error) {
	registry, err := energi_abi.NewIMasternodeRegistryV2(
		energi_params.Range_MasternodeRegistry, w.apiBackend.(bind.ContractBackend))
	if err != nil {
		return nil, nil, err
	}
