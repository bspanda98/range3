...
import (
	...
	energi_testutils "range/core/gen3/energi/common/testutils"
...
func init() {
	...
	energiChainConfig = params.RangeTestnetChainConfig
...
func newTestWorkerBackend(t *testing.T, chainConfig *params.ChainConfig, engine consensus.Engine, n int) *testWorkerBackend {
	...
	case *energi.Range:
		...
		gspec.Alloc[energi_params.Range_MigrationContract] = core.GenesisAccount{Balance: testBankFunds}
		energiEngine := engine.(*energi.Range)
		// energiEngine.testing = true
		energiEngine.SetMinerCB(
			func() []common.Address {
				return []common.Address{energi_params.Range_MigrationContract}
			},
		...
		gspec.Coinbase = energi_params.Range_Treasury
		gspec.Xfers = core.DeployRangeGovernance(chainConfig)
...
func TestPendingStateAndBlockRange(t *testing.T) {
	testPendingStateAndBlock(t, energiChainConfig, energi.New(energiChainConfig.Range, ethdb.NewMemDatabase()))
}
...
// }
func TestEmptyWorkRange(t *testing.T) {
	testEmptyWork(t, energiChainConfig, energi.New(energiChainConfig.Range, ethdb.NewMemDatabase()))
}
...
func TestRegenerateMiningBlockRange(t *testing.T) {
	testRegenerateMiningBlock(t, energiChainConfig, energi.New(energiChainConfig.Range, ethdb.NewMemDatabase()))
}
...
func TestAdjustIntervalRange(t *testing.T) {
	testAdjustInterval(t, energiChainConfig, energi.New(energiChainConfig.Range, ethdb.NewMemDatabase()))
}
