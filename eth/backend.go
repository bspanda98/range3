...
import (
	...
	energi_api "range/core/gen3/energi/api"
...
func New(ctx *node.ServiceContext, config *Config) (*Ethereum, error) {
	...
	log.Info("Initialising Range protocol", "versions", ProtocolVersions, "network", config.NetworkId)

	if !config.SkipBcVersionCheck {
		bcVersion := rawdb.ReadDatabaseVersion(chainDb)
		if bcVersion != nil && *bcVersion > core.BlockChainVersion {
			return nil, fmt.Errorf("database version is v%d, Range Core %s only supports v%d", *bcVersion, params.VersionWithMeta, core.BlockChainVersion)
	...
	var (
		...
		cacheConfig = &core.CacheConfig{Disabled: config.NoPruning, TrieCleanLimit: config.TrieCleanCache, TrieDirtyLimit: config.TrieDirtyCache, TrieTimeLimit: config.TrieTimeout, TrieRapidLimit: config.TrieRapidTime}
	)
	...
	config.TxPool.Protection = ctx.ResolvePath(core.DefaultTxPoolConfig.Protection)
	...
	if energi, ok := eth.engine.(*energi.Range); ok {
...
func CreateConsensusEngine(ctx *node.ServiceContext, chainConfig *params.ChainConfig, config *ethash.Config, notify []string, noverify bool, db ethdb.Database) consensus.Engine {
	...
	if chainConfig.Range != nil {
		return energi.New(chainConfig.Range, db)
	}
...
func (s *Ethereum) Etherbase() (eb common.Address, err error) {
	...
	if wallets := s.AccountManager().Wallets(); len(wallets) > 0 {
		if accounts := wallets[0].Accounts(); len(accounts) > 0 {
			...
			if _, ok := s.engine.(*energi.Range); !ok {
