...
func NewProtocolManager(config *params.ChainConfig, mode downloader.SyncMode, networkID uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb ethdb.Database, whitelist map[uint64]common.Hash) (*ProtocolManager, error) {
	...
	for i, version := range ProtocolVersions {
		...
		if mode == downloader.FastSync && version < nrg70 {
			continue
		}
	...
	manager.downloader = downloader.New(mode, manager.checkpointNumber, chaindb, manager.eventMux, blockchain, nil, txpool, manager.removePeer)
	...
	inserter := func(blocks types.Blocks) (int, error) {
		...
		blocks = manager.txpool.PreBlacklistHook(blocks)
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Range peer", "peer", id)
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Range protocol")
	...
	log.Info("Range protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Range peer connected", "name", p.Name())
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("Range handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Range peer registration failed", "err", err)
		return err
	}
	...
	if err := p.RequestCheckpoints(); err != nil {
		p.Log().Error("Range checkpoint request failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Range message handling failed", "err", err)
			return err
		}
	}
}
