...
import (
	...
	"range/core/gen3/accounts/keystore"
	"range/core/gen3/common"
	"range/core/gen3/common/fdlimit"
	"range/core/gen3/consensus"
	"range/core/gen3/consensus/clique"
	...
	"range/core/gen3/dashboard"
	"range/core/gen3/eth"
	"range/core/gen3/eth/downloader"
	"range/core/gen3/eth/gasprice"
	"range/core/gen3/ethdb"
	"range/core/gen3/ethstats"
	"range/core/gen3/les"
	"range/core/gen3/log"
	"range/core/gen3/metrics"
	"range/core/gen3/metrics/influxdb"
	"range/core/gen3/node"
	"range/core/gen3/p2p"
	"range/core/gen3/p2p/discv5"
	"range/core/gen3/p2p/enode"
	"range/core/gen3/p2p/nat"
	"range/core/gen3/p2p/netutil"
	"range/core/gen3/params"
	whisper "range/core/gen3/whisper/whisperv6"
	cli "gopkg.in/urfave/cli.v1"

	energi "range/core/gen3/energi/consensus"
	energi_svc "range/core/gen3/energi/service"
)
...
var (
	...
	NetworkIdFlag = cli.Uint64Flag{
		Name:  "networkid",
		Usage: "Network identifier (integer, 39797=RangeMain, 49797=RangeTest)",
		Value: eth.DefaultConfig.NetworkId,
	}
	TestnetFlag = cli.BoolFlag{
		Name:  "testnet",
		Usage: "RangeTest network: pre-configured proof-of-work test network",
	}
	...
	MasternodeFlag = cli.BoolFlag{
		Name:  "masternode",
		Usage: "Enable Range Masternode service",
	}
	...
	RangeInitDevFlag = cli.StringFlag{
		Name:  "init",
		Usage: "Range network: use pre-configured custom genesis block",
		Value: "",
	}
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `range3 account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	checkExclusive(ctx, DeveloperFlag, TestnetFlag, RangeInitDevFlag)
	...
	case ctx.GlobalBool(TestnetFlag.Name):
		...
		cfg.Genesis = core.DefaultRangeTestnetGenesisBlock()
	case ctx.GlobalBool(DeveloperFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 1337
		}
	...
	case ctx.GlobalString(RangeInitDevFlag.Name) != "":
		// Check for the pre-defined custom genesis block configuration.
		var err error
		cfg.Genesis, err = core.DeveloperRangeGenesisBlock(ctx.GlobalString(RangeInitDevFlag.Name))
		if err != nil {
			Fatalf("Valid custom genesis block configuration was not found: %v", err)
		}
	default:
		cfg.Genesis = core.DefaultRangeMainnetGenesisBlock()
	}
...
func RegisterEthService(stack *node.Node, cfg *eth.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the Range service: %v", err)
	}
}
...
func RegisterEthStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		Fatalf("Failed to register the Range Stats service: %v", err)
	}
}

// Configure Range Dynamic Checkpoint service
func RegisterDynamicCheckpointService(stack *node.Node) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		Fatalf("Failed to register the Range Checkpoint service: %v", err)
	}
}

// RegisterMasternodeService configures Range Masternode service. It also accepts
...
func RegisterMasternodeService(stack *node.Node, owner common.Address) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		Fatalf("Failed to register the Range Masternode service: %v", err)
	}
}
...
func MakeChain(ctx *cli.Context, stack *node.Node) (chain *core.BlockChain, chainDb ethdb.Database) {
	...
	if config.Range != nil {
		engine = energi.New(config.Range, chainDb)
