...
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppeth, your Range private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new Range network down to  |")
	...
	log.Info("Administering Range network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".energipuppeth", w.network)
