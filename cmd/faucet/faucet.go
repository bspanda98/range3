...
var (
	...
	ethPortFlag = flag.Int("ethport", 39797, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Range protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Range and the faucet")
)
...
func main() {
	...
	ks := keystore.NewKeyStore(filepath.Join(os.Getenv("HOME"), ".energifaucet", "keys"), keystore.StandardScryptN, keystore.StandardScryptP)
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	stack, err := node.New(&node.Config{
		...
		DataDir: filepath.Join(os.Getenv("HOME"), ".energifaucet"),
		P2P: p2p.Config{
			NAT:              nat.Any(),
			NoDiscovery:      true,
			DiscoveryV5:      true,
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://range/core/gen3/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Range address found to fund")
	}
	var avatar string
