Clef
----
Clef can be used to sign transactions and data and is meant as a replacement for range3's account management.
This allows DApps not to depend on range3's account management. When a DApp wants to sign data it can send the data to
the signer, the signer will then provide the user with context and asks the user for permission to sign the data. If
the users grants the signing request the signer will send the signature back to the DApp.
  
This setup allows a DApp to connect to a remote Range node and send transactions that are locally signed. This can
help in situations when a DApp is connected to a remote node because a local Range node is not available, not
synchronised with the chain or a particular Range node that has no built-in (or limited) account management.
...
GLOBAL OPTIONS:
   ...
   --networkid value       Network identifier (integer, 39797=RangeMain, 49797=RangeTest) (default: 39797)
...
The general flow for signing a transaction using e.g. range3 is as follows:
![image](sign_flow.png)

In this case, `range3` would be started with `--externalsigner=http://localhost:8550` and would relay requests to `eth.sendTransaction`.
...
put together is a bit of a hack into the http server. This could probably be greatly improved
    - Relay: Range Core should be started in `range3 --external_signer localhost:8550`.
    - Currently, the Range Core APIs use `common.Address` in the arguments to transaction submission (e.g `to` field). This
  ...
  retains the original input.
    - The Range Core api should switch to use the same type, and relay `to`-account verbatim to the external api.
...
* Another potential thing to introduce is pairing.
  ...
  * Thus range3/mist/cpp would cryptographically handshake and afterwards the caller would be allowed to make signing requests.
...
The signer listens to HTTP requests on `rpcaddr`:`rpcport`, with the same JSONRPC standard as Range Core. The messages are
